...
class BaseTrainer:
    ...
    def train(self, train_dataset, valid_dataloader, model):
        ...
        self.model = torch.nn.parallel.DataParallel(
            model, device_ids=list(range(world_size))
        )
        ...
        if self.multilabel:
            criterions = [{"loss": nn.MultiLabelSoftMarginLoss().cuda(), "weight": 1}]
        else:
            criterions = [
                {"loss": nn.CrossEntropyLoss(weight=weights).cuda(), "weight": 1}
            ]
        ...
        for epoch in range(start_epoch, self.epochs + 1):
            ...
            if epoch % 5 == 0:
                ...
                if epoch_accuracy > best_accuracy:
                    ...
                    print(f"Saving best model with accuracy {epoch_accuracy}")
        cleanup()

...
class CKA:
    ...
    def _log_layer(
        self, model: str, name: str, layer: nn.Module, inp: torch.Tensor, out: torch.Tensor
    ):
    ...
    def compare(self, dataloader1: DataLoader, dataloader2: DataLoader = None) -> None:
        ...
        with torch.no_grad():
            ...
            self.hsic_matrix = torch.zeros(N, M, 3).to(self.device)

            num_batches = min(len(dataloader1), len(dataloader1))

            for (x1, *_), (x2, *_) in tqdm(
                zip(dataloader1, dataloader2), desc="| Comparing features |", total=num_batches
            ):
                ...
                for i, (name1, feat1) in enumerate(self.model1_features.items()):
                    ...
                    self.hsic_matrix[i, :, 0] += self.linear_HSIC(X, X) / num_batches
                    ...
                    for j, (name2, feat2) in enumerate(self.model2_features.items()):
                        ...
                        self.hsic_matrix[i, j, 1] += self.linear_HSIC(X, Y) / num_batches
                        self.hsic_matrix[i, j, 2] += self.linear_HSIC(Y, Y) / num_batches
    ...
    def plot_results(self, save_path: str = None, title: str = None):
        ...
        df = pd.DataFrame(self.hsic_matrix.cpu().numpy())
        df.columns = self.model2_info["Layers"]
        df.index = self.model1_info["Layers"]
        ax = sns.heatmap(df, annot=True, fmt=".2f")
        ax.set_xlabel(f"Layers {self.model2_info['Name']}", fontsize=15)
        ax.set_ylabel(f"Layers {self.model1_info['Name']}", fontsize=15)
        plt.yticks(rotation=0)
        # fig, ax = plt.subplots()
        # im = ax.imshow(self.hsic_matrix.cpu().numpy(), origin="lower", cmap="magma")
        # ax.set_xlabel(f"Layers {self.model2_info['Name']}", fontsize=15)

...
df = pd.read_csv("stats/final_stats/bf_stats_tif.csv")
save_path = (
    "/proj/haste_berzelius/exps/specs_new_splits/dataset_exploration/bf_well_stats"
)
...
for plate in df.plate.unique():
    ...
    means.append(
        df_plate[
            (df_plate.compound == "dmso") & (df_plate.well == "all")
        ].mean_C1.values[0]
    )
    stds.append(
        df_plate[
    ...
    for idx, row in df_plate.iterrows():
        if row.well != "all":
            ...
            well = row.well
            idx = np.where((df_plate.well == "all") & (df_plate.compound == compound))[
                0
            ][0]
            ...
            heatmap[row_idx, col_idx] = row["std_C1"]
            #  row[
            #     "mean_C1"
    ...
    plt.savefig(f"{save_path}/{plate}_std_heatmap.png")
    plt.close()
plt.figure(figsize=(16, 12))
plt.errorbar(
    plates, means, stds, fmt="ok", lw=15, alpha=0.5, label="mean", ecolor="blue",
)

...
class ResNet(nn.Module):
    def __init__(self, model_name, in_channels=3, num_classes=1000):
        ...
        self.relu = nn.ReLU(inplace=False)  # inplace=True)
...
class ResNetSSL(nn.Module):
    def __init__(self, model_name, in_channels=3):
        ...
        self.projector = nn.Sequential(
            nn.Linear(self.inplanes, self.inplanes),
    ...
    def forward(self, x):
        feat = self.forward_features(x)
        op = F.normalize(self.projector(feat), dim=1)
        return op, feat


class ResNetLRP(nn.Module):
    def __init__(self, model_name, in_channels=3, num_classes=1000):
        super(ResNetLRP, self).__init__()

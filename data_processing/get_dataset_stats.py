...
def get_dataset_stats_mp(image_path, all_dataset_df, mode):
    with mp.Pool(16) as pool:  # use 3 processes
        ...
        results = []
        for i in range(n):
            results.append(pool.apply_async(get_plate_stats, args=(plate_dfs[-i], mode)))
...
def get_plate_stats(df_plate, mode):
    plates = []
    compounds = []
    wells = []
    sites = []
